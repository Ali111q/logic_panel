    ...
    async function fetchData() {
      try {
        if (!token) {
          ...
          const response = await axios.get(`http://172.20.10.11:5000/api/university/${universityId}`, { headers });
  ...
  const universityUpdateHandler = async (e) => {
    ...
    try {
      if (!token) {
        ...
        const response = await axios.patch(`http://172.20.10.11:5000/api/university/${universityId}`, {
          universityName,
          universityImgUrl
        }, { headers });
